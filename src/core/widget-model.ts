...
export const WIDGET_PRESETS: ReadonlyArray<Readonly<WidgetPresetDefinition>> = Object.freeze([
  ...
  { id: "tasks.unscheduled", title: "未安排任务", description: "尚未设置计划日期或截止日期的任务", typeId: "view.list", config: config("tasks", { query: { mode: "unscheduled", limit: 15 }, actions: ["complete", "schedule", "edit"] }) },
  ...
  { id: "tasks.calendar", title: "任务日历", description: "按截止日期查看任务期限", typeId: "view.calendar", config: config("tasks") },
  { id: "schedule.overview", title: "日程总览", description: "未来七天的计划任务、会议与 Full Calendar 日程", typeId: "view.calendar", config: config("mixed", { query: { limit: 100 } }) },
