...
5. 「启动时打开 Asterism」默认启用：工作区就绪后自动复用或打开工作台，不关闭其他笔记。
6. 原 `Workbench.md` 和 `Workbench Panel.md` 不会自动删除，可继续作为回退入口。
...
工作台的「新建项目笔记」使用分区表单，默认保存到 `10_业务_Business/04_方案资产_SolutionAssets`；
项目优先、客户补充，并支持按名称、状态、元数据或路径输入搜索。会议和供应商等关联对象
也使用同一套可检索选择器，不再依赖长下拉菜单。
所有工作台弹窗共用同一套视口内滚动、字段尺寸与操作区规则，长表单不会再溢出弹窗。
项目审阅只选择一次审阅结论，项目状态由结论自动联动；审阅页可直接完成、改期或调整
未完成任务的优先级，并同时呈现关联会议与方案资产。
工作台采用四列有序卡片网格，列宽与行高分别计算：`1×1` 是紧凑卡片，不再被强制成
近似正方形。速记、快捷操作和活动热力图的新建实例默认只占一行；升级到 `0.7.0`
时也会安全压缩这些旧实例，完整迁移前布局仍保留为兼容备份。
...
侧栏会按工作台、任务看板、项目、客户、供应商、会议、知识和普通笔记自动切换独立布局；
在命令面板运行「Asterism: 打开上下文侧栏」即可打开，之后可固定在右侧。每个上下文布局
都可以在设置页单独调整，不会改写当前笔记或业务模板。

今日执行中的「今日焦点」支持全部、逾期、今天和高优先快捷视图，并可按任务来源、
项目类型、客户、优先级和完成状态组合筛选。客户筛选使用可搜索、多选的选择器；
筛选条件只保存在布局配置中，不写入业务 Markdown。

「速记」使用独立的 `Asterism 速记.md`。每次输入会作为独立条目，
...
日程联动使用 Full Calendar Remastered 的公开授权 API，不读取其私有内部状态。
「日程总览」合并 Tasks 计划、会议日期和 Full Calendar 外部事件；外部事件只读，
点击后进入完整日历。任务的「安排」操作只写入 Tasks 的 ⏳ 计划日期，原有 📅 截止日期
保持不变；需要具体时间块时，在 Full Calendar 中继续拖放。首次使用会显示授权请求，
Full Calendar 未安装、未授权或暂时停用时，Asterism 的项目、任务和会议功能仍可独立运行。

独立「项目审阅」页面可通过 Ribbon、工作台 Hero、侧栏快捷入口或命令打开。页面按
主动送审、到期复审、任务逾期或缺少下一步建立候选队列，一次集中展示项目事实、健康信号、任务、
等待项与关联会议。选择结论后先显示写入预览；确认后只更新目标项目的审阅字段，并在
既有「推进记录」追加一条可追溯记录。页面中的「YOLO 审阅」会复制当前页面的结构化字段、
任务摘要、会议标题和统计，并打开当前项目的 YOLO 新对话。`project-review` Skill 默认启用；
粘贴发送后可以持续追问、补充证据和讨论结论，不再依赖一次性 Agent API。对话默认只读，
需要修改时必须先给出差异预览并由用户确认。
项目、会议和审议模板始终保持只读。
