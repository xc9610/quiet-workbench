...
工作台包含今日执行、项目管理、知识整理三套默认布局。常规模式只显示业务组件；
点击顶部「编辑布局」后才显示添加、拖动、缩放、设置、折叠和隐藏操作。桌面端可
拖动和缩放，移动端使用排序、折叠和隐藏。布局撤销与业务事务撤销相互独立。
...
默认今日布局用「速记」替代了与今日焦点重复的近期截止列表。每次输入会作为独立条目，
